...
if "df" not in st.session_state:
    st.warning("Najpierw wczytaj dane w zakładce głównej.")
    st.stop()
...
st.markdown("### 🌳 Schemat przepływu danych")

dot = graphviz.Digraph()

# Węzły źródłowe
dot.node("FactOnlineSales", shape="box", style="filled", color="lightblue")
dot.node("DimCustomer")
dot.node("DimProduct")
dot.node("DimOrderChannel")
dot.node("DimPaymentMethod")
dot.node("DimDeliveryMethod")
dot.node("DimSalesTerritory")

# Węzeł wyjściowy
dot.node("Zbiór końcowy", shape="ellipse", style="filled", color="lightgreen")

# Połączenia
dot.edges([
    ("FactOnlineSales", "Zbiór końcowy"),
    ("DimCustomer", "Zbiór końcowy"),
    ("DimProduct", "Zbiór końcowy"),
    ("DimOrderChannel", "Zbiór końcowy"),
    ("DimPaymentMethod", "Zbiór końcowy"),
    ("DimDeliveryMethod", "Zbiór końcowy"),
    ("DimSalesTerritory", "Zbiór końcowy"),
])
